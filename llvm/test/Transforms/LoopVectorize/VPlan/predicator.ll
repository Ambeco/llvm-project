...
; CHECK-EMPTY:
; CHECK-NEXT:    latch:
; CHECK-NEXT:      WIDEN-PHI vp<[[VP8:%[0-9]+]]> = phi [ vp<[[VP4:%[0-9]+]]>, F ], [ ir<false>, D ], [ ir<false>, C ]
; CHECK-NEXT:      WIDEN-PHI vp<[[VP9:%[0-9]+]]> = phi [ ir<false>, F ], [ ir<%c1>, D ], [ ir<%c1>, C ]
; CHECK-NEXT:      BLEND ir<%phi> = ir<%y>/vp<[[VP8]]> ir<%x>/vp<[[VP9]]>
; CHECK-NEXT:      EMIT ir<%gep> = getelementptr ir<%p>, ir<%iv>
; CHECK-NEXT:      EMIT store ir<%phi>, ir<%gep>
; CHECK-NEXT:      EMIT ir<%iv.next> = add ir<%iv>, ir<1>
; CHECK-NEXT:      EMIT ir<%ec> = icmp eq ir<%iv.next>, ir<128>
...
; CHECK-EMPTY:
; CHECK-NEXT:    bb3:
; CHECK-NEXT:      WIDEN-PHI vp<[[VP5:%[0-9]+]]> = phi [ ir<poison>, bb1 ], [ ir<%add2>, bb2 ]
; CHECK-NEXT:      WIDEN-PHI vp<[[VP6:%[0-9]+]]> = phi [ ir<false>, bb1 ], [ ir<true>, bb2 ]
